...
export async function createGap(_prev: unknown, formData: FormData): Promise<{ success?: boolean; error?: string }> {
  ...
  if (!employeeId || !description) return { error: 'Colaborador e descrição obrigatórios.' };

  const supabase = await createClient();
  const { error } = await supabase.from('training_gaps').insert({ employee_id: employeeId, description, severity });
